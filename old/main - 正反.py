...
num = 0
for i in os.listdir():
    if i.endswith('.jpg') and int(re.findall(r'_(.*?)\.jpg', i)[0]) % 2:
        tmp = read_barcodes(os.path.join('.', i))
        if tmp:
            img[tmp[0]['data'] + 'a'] = i
            img[tmp[0]['data'] + 'b'] = i.split('_')[0] + '_' + str(int(re.findall(r'_(.*?)\.jpg', i)[0]) + 1) + '.jpg'
        else:
            if not os.path.exists('无条码'):
                os.makedirs('无条码')
            shutil.copy(i, os.path.join('无条码', i))
            shutil.copy(i.split('_')[0] + '_' + str(int(re.findall(r'_(.*?)\.jpg', i)[0]) + 1) + '.jpg'
                        , os.path.join('无条码', i.split('_')[0] + '_' + str(int(re.findall(r'_(.*?)\.jpg', i)[0]) + 1) + '.jpg'))
        num += 1
        if num%10 == 0:
            print(num)
